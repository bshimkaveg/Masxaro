Welcome to the Masxaro Portal - A Gatein Application
====================================================

This document explains how to build and package it with JBoss.


Prerequisites
=============

- Java Development Kit 1.6 (Version 1.7 does not currently work)
- We recommend Fedora 16 as Fedora 17 only support Java 1.7
- Recent Subversion client
- Recent Maven 3 (might work with Maven 2.2.1 as well)


Procedure for Jboss AS-7
=======================
...
Package the Portal  with JBoss-AS-7.1.0.Final
-----------------------------------

If you don’t have an existing JBoss AS distribution, the build can automatically download it for you.

2a. From the Masxaro/Portal directory, issue the following command:

       mvn install -DskipTests -Dservers.dir=/var/jboss -Dgatein.dev=jbossas7 -Ddownload


If you have an existing JBoss-AS-7.1.0.Final distribution, unpack it into /var/jboss directory so that you get var/jboss/jboss-as-7.1.0.Final directory.  

2b. From the Masxaro/Portal directory, issue the following command:

       mvn install -DskipTests -Dservers.dir=/var/jboss -Dgatein.dev=jbossas7



*Note:  This will create a generic AS-7 instance in /var/jboss/.   You will need certain modules and a gate-in modified standalone.xml included in the gatein build to get the portal to work.

** Note: When the package builds, it will be found in Masxaro/Portal/packaging/jboss-as7/pkg/target/jboss-as-7.1.0.Final.


3. Copy the contents of Masxaro/Portal/packaging/jboss-as7/pkg/target/jboss-as-7.1.0.Final to /var/jboss

* Note: The build might cause the files to compile as an userid:groupid = root:root.  Ensure you change this to an userid:groupid consistent with your permissions.

** Note: Ensure var/jboss/jboss-as-7.1.0/bin/standalone.sh is executable.  Use chmod 755 for the files in /var/jboss/jboss-as-7.1.0/bin
 
4. Add Jboss manaagement access with /var/jboss/jboss-as-7.1.0.Final/bin/adduser.sh and follow the instructions.

5. Start Marsxaro Portal with /var/jboss/jboss-as-7.1.0.Final/bin/standalone.sh

6. Access the portal at: http://localhost:8080/portal

7. Manage AS-7 at http://localhost:9990 

*****

After modifications to the src files:

1. Compile at the Masxaro/Portal directory level.
2. Navigate to the Masxaro/Portal/packaging/jboss-as7/pkg/target/jboss-as-7.1.0.Final/standalone/deployments section.
3. Ensure the contents are not assigned to root:root
4. Ensure the EAR & WAR files have ear.dodeploy and war.dodeploy extensions included in the directory.
5. Copy these files to /var/jboss/jboss-as-7.1.0.Final/standalone/deployments directory.
6. With the application server running, the scanner should pick them up.
7. If there are problems, restart the server to get the changes picked up.
 

General Gatein Build configuration
===================
